...
class Board:
    def __init__(self, play_status):
        self.board = [] #6x6 board with info about checker pieces
        ...
        self.board[1][0][1] = CheckerPiece("white", 1, 0, "A0")
        self.board[0][1][1] = CheckerPiece("white", 0, 1, "A1")
        self.board[1][2][1] = CheckerPiece("white", 1, 2, "A2")
        self.board[0][3][1] = CheckerPiece("white", 0, 3, "A3")
        self.board[1][4][1] = CheckerPiece("white", 1, 4, "A4")
        self.board[0][5][1] = CheckerPiece("white", 0, 5, "A5")

        #black checker pieces (player) insertion
        self.board[5][0][1] = CheckerPiece("black", 5, 0, "P0")
        self.board[4][1][1] = CheckerPiece("black", 4, 1, "P1")
        self.board[5][2][1] = CheckerPiece("black", 5, 2, "P2")
        self.board[4][3][1] = CheckerPiece("black", 4, 3, "P3")
        self.board[5][4][1] = CheckerPiece("black", 5, 4, "P4")
        self.board[4][5][1] = CheckerPiece("black", 4, 5, "P5")
        
        self.play_status = play_status #1 is player's turn, 2 is computer's turn


    '''returns a list of legal moves for each AI checker piece:
       [ for each CheckerPiece[ piece_id, [legal moves]]]'''
    def AI_legal_moves(self, init_board):
        leg_moves = []
    ...
    '''returns a list of legal moves for each player checker piece:
       [ for each CheckerPiece[ piece_id, [legal moves]]]'''
    def player_legal_moves(self, init_board):
        leg_moves = []
        for i in range(len(init_board)):
            for j in range(len(init_board[i])):
                if init_board[i][j][0] != False and init_board[i][j][1] != None:
    ...
    for i in range(len(board)):
        ...
        for j in range(len(board[i])):
            if board[i][j][0] is False:
                ...
                line += "b"
        print(line)
    print(game.AI_legal_moves(board))
    print(game.player_legal_moves(board))
